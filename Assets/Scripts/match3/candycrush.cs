...
public class candycrush : MonoBehaviour
{
    ...
    IEnumerator SwapAnimation(GameObject tile1,GameObject tile2,float duration)
    {
        ...
        Vector2 t1t = tile1.transform.position;
        GameObject fake1 = (GameObject)Instantiate(Resources.Load("match3prafabs/"+t1),t1t,Quaternion.identity);
    ...
    IEnumerator DispareAnimation(GameObject tileObj,int duration,int c,int i,int r)
    {
        ...
        GameObject fake = (GameObject)Instantiate(Resources.Load("match3prafabs/"+type),tt,Quaternion.identity);
        ...
        {
            percent += Time.deltaTime / duration;
            fake.transform.localScale = Vector2.Lerp(fake.transform.localScale, endScale, percent);
            yield return null;
        }
            ...
            for (int c = 0; c < cols; c++)
            {
                ...
                //if in the top row and no tile
                {
                    ...
                    for (int n = 0; n < tileBank.Count; n++)
                    {
                        ...
                        if (!o.activeSelf)
                        {
                            o.transform.position = new Vector2(
                                tilePos.x, tilePos.y
                                );
                            o.SetActive(true);
                            tiles[c, r] = new Tile(o, o.name);
